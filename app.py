...
def send_info_to_mqtt():
    ...
    while True:
        ret = client1.publish("house/wifi", networks.reset_index().to_json(orient='records'))  # publish
        # print(ret)
        networks.drop(networks.index, inplace=True)  # drop all entries in networks after they've been send
        time.sleep(60)
